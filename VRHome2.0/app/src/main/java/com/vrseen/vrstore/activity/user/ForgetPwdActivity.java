...
public class ForgetPwdActivity extends BaseActivity {
    ...
    private void stopCountDown() {
        ...
        _btnGetcode.setText(getResources().getString(R.string.get_retry));
    }
    ...
    private void requestForgetPwd()
    {
        ...
        if(_pwd.length() < 6){
            ToastUtils.showShort(_context, getResources().getString(R.string.password_is_short_hint));
            return;
        }
        UserRestClient.getInstance(this).findPwdByPhone(_strPhone,_pwd,_code,new AbstractRestClient.ResponseCallBack() {
            @Override
            public void onFailure(Response resp, Throwable e) {
                CommonUtils.showResponseMessage(_context,resp,e,R.string.mine_find_pwd_fail);
            }

            @Override
            public void onSuccess(Response responseInfo) {
                if (responseInfo != null) {
                    ToastUtils.showShort(_context,R.string.mine_find_pwd_success);
                }

                UserLogic.getInstance().loginByPhone(ForgetPwdActivity.this,_strPhone,_pwd);
            }
        });
    }
    ...
    private void mobileCheck()
    {
        UserRestClient.getInstance(_context).mobileCheck(_strPhone, new AbstractRestClient.ResponseCallBack() {
            @Override
            public void onFailure(Response resp, Throwable e) {
                CommonUtils.showResponseMessage(_context,resp,e,R.string.message_server_1001010);
            }
            ...
            public void onSuccess(Response resp) {
                ...
                try {
                    ...
                    if(status == 0)
                    {
                        ToastUtils.showShort(_context,R.string.message_server_1001010);
                    }
                    else
                    {
                        requestCode();
    ...
    private void  requestCode()
    {
        ...
        startCountDown();
        //获取验证码和倒计时
        UserRestClient.getInstance(this).requestGetCode(_etPhone.getText().toString(),UserRestClient.REGISTER_FORGET_PWD, new AbstractRestClient.ResponseCallBack() {
